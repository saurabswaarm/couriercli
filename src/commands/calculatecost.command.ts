...
import { z } from 'zod';
import { DeliveryCostInputSchema, InitialInputSchema, PackageSchema } from '../schemas/package.schema';
...
export class CalculateCostCommand {
  ...
  private async promptInitialDetails(): Promise<void> {
    try {
      ...
      const [baseDeliveryCostStr, numberOfPackagesStr] = initialAnswer.initialDetails.trim().split(/\s+/);
      
      const parsedInput = InitialInputSchema.parse({
  ...
  private async promptSinglePackageDetails(packageNumber: number): Promise<void> {
    try {
      ...
      const parts = packageAnswer.packageDetails.trim().split(/\s+/);
      const packageId = parts[0];
      const weight = Number(parts[1]);
      const distance = Number(parts[2]);
      const offerCode = parts.length > 3 ? parts[3] : undefined;
